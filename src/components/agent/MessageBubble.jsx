...
const FunctionDisplay = ({ toolCall }) => {
    ...
    const statusConfig = {
        ...
        in_progress: { icon: Loader2, color: 'text-slate-500', text: 'בתהליך...', spin: true },
        completed: isError ? 
            { icon: AlertCircle, color: 'text-red-500', text: 'נכשל' } : 
            { icon: CheckCircle2, color: 'text-green-600', text: 'הצליח' },
        success: { icon: CheckCircle2, color: 'text-green-600', text: 'הצליח' },
    ...
    return (
        <div className={cn("flex gap-3", isUser ? "justify-end" : "justify-start")}>
            ...
            <div className={cn("max-w-[85%]", isUser && "flex flex-col items-end")}>
                            ...
                            <ReactMarkdown 
                                ...
                                components={{
                                    code: ({ inline, className, children, ...props }) => {
                                        ...
                                        return !inline && match ? (
                                            <div className="relative group/code" dir="ltr">
                                                ...
                                                <Button
                                                    size="icon"
                                                    variant="ghost"
                                                    className="absolute top-2 left-2 h-6 w-6 opacity-0 group-hover/code:opacity-100 bg-slate-800 hover:bg-slate-700"
                                                    onClick={() => {
                                    ...
                                    blockquote: ({ children }) => (
                                        <blockquote className="border-r-2 border-slate-300 pr-3 my-2 text-slate-600">
                                            {children}
                                        </blockquote>
                                    ),
                                }}
             ...
             {isUser && (
                <div className="h-8 w-8 rounded-lg bg-blue-600 flex items-center justify-center mt-0.5 text-white font-semibold text-sm shrink-0">
                   אני
                </div>
            )}
        </div>
