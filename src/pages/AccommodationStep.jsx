...
export default function AccommodationStep() {
  ...
  const { actualCost, setActualCost } = useActualCost(trip, "accommodation_details");
  ...
  const loadTrip = async () => {
    ...
    if (guestMode) {
      const migrated = await transitionGuestToUser(navigate, 'AccommodationStep');
      if (migrated) return;
    }
  ...
  const markStepCompleted = async () => {
    ...
    if (isGuestMode) {
      ...
      if (savedDraft) {
        ...
        applyActualCostToDraft(draftData, "accommodation_details", actualCost);
    ...
    try {
      const acPayload = await buildActualCostPayload(trip, "accommodation", "accommodation_details", actualCost);
